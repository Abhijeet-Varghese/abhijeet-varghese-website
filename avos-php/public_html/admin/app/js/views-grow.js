...
(() => {
  ...
  R.after("leads", view => {
    ...
    const loadTrash = async () => {
      const r = await AV.api.get("/api/leads?limit=100");
      const trashed = (r.data && r.data.total) || 0;
      const tb = $("[data-trash]", view);
      if (tb) tb.innerHTML = `${icon("trash")} Trash`;
    };
    const render = () => {
  ...
  R.register("analytics", () => {
    ...
    const total = a.visitors30.reduce((x, y) => x + y, 0);
    const donut = (data, size = 130) => {
      const sum = data.reduce((x, [, v]) => x + v, 0);
      let acc = 0;
      const cols = ["var(--accent)", "var(--azure)", "var(--ok)", "var(--warn)", "var(--ink-4)"];
      ...
      return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="transform:rotate(-90deg)">
        ${data.map(([, v], i) => {
          ...
          const off = -acc * c;
          acc += frac;
          return `<circle cx="${size/2}" cy="${size/2}" r="${r}" fill="none" stroke="${cols[i]}" stroke-width="14" stroke-dasharray="${dash}" stroke-dashoffset="${off}"/>`;
        }).join("")}
      </svg>`;
    };
    const bars = (data, max = 60) => data.map(([l, v], i) => `
      <div style="display:flex;align-items:center;gap:10px;padding:5px 0;font-size:12.5px">
        <span style="width:110px;color:var(--ink-2)">${esc(l)}</span>
        <div class="prog" style="flex:1"><i style="width:${(v / Math.max(...data.map(d => d[1]))) * 100}%"></i></div>
        <b style="width:40px;text-align:right">${v}%</b>
      </div>`).join("");
    return `
    <div class="view__head">
      <div><h1 class="view__title">Analytics</h1>
      <p class="view__desc">GA4 · Search Console · Clarity · Cloudflare — unified in one calm view.</p></div>
      <div class="view__head-actions">
        <div class="seg"><button>30 days</button><button class="is-active">90 days</button><button>12 months</button></div>
        <button class="btn btn--ghost" data-export>${icon("download")} Export</button>
      </div>
    </div>
    <div class="grid grid-4" style="margin-bottom:16px">
      ${[["Visitors", total.toLocaleString(), "+14.2%"], ["Page views", "46,932", "+18.1%"], ["Avg. session", "3m 42s", "+0.4%"], ["Bounce rate", "41.2%", "−2.8%"]].map(([l, v, t]) => `
        <div class="card card--hover stat" style="padding:16px 18px">
          <p class="stat__value" style="font-size:1.5rem">${v}</p>
          <p class="stat__label">${l}</p>
          <span class="stat__trend up" style="align-self:flex-start;margin-top:4px">${t}</span>
        </div>`).join("")}
    </div>
    <div class="grid grid-13" style="margin-bottom:16px">
      <div class="card">
        <div class="card__head"><p class="card__title">Visitors · last 30 days</p><span class="chip chip--ok">Live</span></div>
        <div class="card__body" style="padding-top:12px">
          <svg class="chart-svg" viewBox="0 0 520 150" preserveAspectRatio="none" style="height:150px;width:100%">
            <defs><linearGradient id="ag2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="var(--azure)" stop-opacity="0.3"/><stop offset="100%" stop-color="var(--azure)" stop-opacity="0"/></linearGradient></defs>
            <polygon points="0,150 ${a.visitors30.map((v, i) => `${(i / 29) * 520},${150 - (v / Math.max(...a.visitors30)) * 140}`).join(" ")} 520,150" fill="url(#ag2)"/>
            <polyline points="${a.visitors30.map((v, i) => `${(i / 29) * 520},${150 - (v / Math.max(...a.visitors30)) * 140}`).join(" ")}" fill="none" stroke="var(--azure)" stroke-width="2.2"/>
          </svg>
        </div>
        <div class="card__foot"><span class="chip chip--muted">GA4 connected</span><span class="chip chip--muted">Clarity session recordings on</span></div>
      </div>
      <div class="card">
        ...
        <div class="card__body" style="display:flex;gap:20px;align-items:center;flex-wrap:wrap">
          <div>${donut(a.sources)}</div>
          <div style="flex:1;min-width:160px">${bars(a.sources)}</div>
        </div>
      </div>
    </div>
    <div class="grid grid-2" style="margin-bottom:16px">
      <div class="card">
        <div class="card__head"><p class="card__title">Top countries</p></div>
        <div class="card__body">${bars(a.countries)}</div>
      </div>
      <div class="card">
        <div class="card__head"><p class="card__title">Devices</p></div>
        <div class="card__body">
          <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">
            <div style="display:flex;gap:14px;align-items:center">
              ${[["Mobile", "var(--accent)"], ["Desktop", "var(--azure)"], ["Tablet", "var(--ok)"]].map(([l, c], i) => `
                <div style="display:grid;gap:4px;justify-items:center">
                  <div style="width:44px;height:44px;border-radius:12px;background:${c}22;color:${c};display:grid;place-items:center;font-size:17px;font-weight:700">${["📱", "💻", "📟"][i]}</div>
                  <span style="font-size:11px;color:var(--ink-3)">${l}</span>
                  <b style="font-size:12px">${a.devices[i][1]}%</b>
                </div>`).join("")}
            </div>
          </div>
        </div>
    ...
    <div class="grid grid-2">
      <div class="card">
        <div class="card__head"><p class="card__title">Popular pages</p></div>
        <div class="card__body">${bars(a.pages)}</div>
      </div>
      <div class="card">
        <div class="card__head"><p class="card__title">Heatmaps & scroll depth</p><span class="chip chip--accent">Clarity</span></div>
        <div class="card__body">
          <div style="display:flex;gap:10px;align-items:flex-end;height:110px">
            ${[42, 68, 84, 92, 76, 58, 40, 26, 14].map((h, i) => `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px">
              <div style="width:100%;height:${h}%;border-radius:6px 6px 2px 2px;background:linear-gradient(180deg,var(--accent),var(--azure));opacity:${0.35 + (h / 100) * 0.65}"></div>
              <span style="font-size:9px;color:var(--ink-4)">${i * 10}%</span>
            </div>`).join("")}
          </div>
          <p style="font-size:11.5px;color:var(--ink-3);margin-top:10px">Scroll depth: 84% of visitors reach the work section · 41% reach contact.</p>
        </div>
      </div>
    </div>`;
  });
  R.after("analytics", view => {
    $("[data-export]", view).addEventListener("click", () => toast("Analytics exported — CSV", "accent"));
  });
})();
