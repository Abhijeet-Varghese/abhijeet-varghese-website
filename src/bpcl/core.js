/* BPCL micro-app engine module: core.js — ported (IIFE unwrapped where
   present, asset paths rebased to /assets/bpcl/). Called in legacy order. */
export default function initBpclCore() {
  ...
  APP.path = function (file) {
    ...
    return {
      dir: '/assets/bpcl/images/' + group + '/' + file,
      srcset: widths.map(function (w) {
        return '/assets/bpcl/images/' + group + '/' + file + '-' + w + '.webp ' + w + 'w';
      }).join(', '),
      fallback: '/assets/bpcl/images/' + group + '/' + file + '-1280.jpg',
