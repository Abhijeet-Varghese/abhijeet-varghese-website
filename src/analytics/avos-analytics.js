/* AV OS first-party analytics — ported verbatim from the legacy inline
   module (identical across all 24 legacy pages). Privacy-respecting:
   pageview/scroll_depth/cta/external/download/view/video/gallery/search. */
...
(function () {
  ...
  (function () {
    ...
    else if ((m = p.match(/\/case-studies\/[^\/]+\/?/)) || (m = p.match(/\/case-studies\//)) || (m = p.match(/\/case-study-[^\/]+\.html/)) || (m = p.match(/\/experience-design\/[^\/]+\/?/))) { avTrack({ event_type: "case_study_view", path: p, content: m[0] }); }
  ...
  /* SPA bridge: router calls pageview() on navigation; listeners below are
     document-delegated and bind once at boot. */
  window.AVAnalytics = {
    track: function (extra) { avTrack(extra || {}); },
    pageview: function (path) { avTrack({ event_type: "pageview", path: path || location.pathname }); }
  };
})();