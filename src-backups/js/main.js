  ...
  if (navToggle && mobileMenu) {
    ...
    const setMenu = (open, restoreFocus = true) => {
      ...
      if (!open) {
        ...
        cancelAnimationFrame(menuOpenInnerFrame);
        menuOpenFrame = 0;
        menuOpenInnerFrame = 0;
      }
      if (open) {
        ...
        menuOpenFrame = requestAnimationFrame(() => {
          menuOpenInnerFrame = requestAnimationFrame(() => {
            if (navToggle.getAttribute("aria-expanded") === "true" && !mobileMenu.hidden) {
    ...
    const closeOnDesktop = () => { if (!mobileMQ.matches) setMenu(false); };
