/* ============================================================
   ORANGE BUSINESS — v7.0 "SIGNAL IN THE DARK"
   One IIFE. IO-driven, transform/opacity only (§29).
   ============================================================ */
(() => {
  "use strict";
  ...
  const selectPin = (pin) => {
    ...
    if (panoNum) panoNum.textContent = pin.dataset.n;
    if (panoTitle) panoTitle.textContent = pin.dataset.title;
    if (panoDesc) panoDesc.textContent = pin.dataset.desc;
    if (panoText) {
      panoText.classList.remove("is-swap");
      void panoText.offsetWidth;
      panoText.classList.add("is-swap");
    }
    pins.forEach((p) => { if (p !== pin) p.removeAttribute("aria-current"); });
  ...
  if (pins.length && panoTitle) {
    ...
    if (tourAllowed) {
      ...
      if (panoFig && "IntersectionObserver" in window) {
        ...
        }, { threshold: 0.5 }).observe(panoFig);
      }
    }
  }

  /* ---------- 4. §04 stage switcher — text only ---------- */
  ...
  if (stageBtns.length && stagePanel) {
    ...
    stageBtns.forEach((btn) => btn.addEventListener("click", () => {
      ...
      stageBody.innerHTML =
        '<span class="ob-stages__tag">' + btn.dataset.tag + "</span>" +
        "<h3>" + btn.dataset.i + " — " + btn.dataset.title + "</h3>" +
  ...
  /* ---------- 5. §05 ENVIRONMENTAL RESPONSE — original module, single toggle ---------- */
  const room = document.querySelector(".room-response");
  const roomToggle = document.querySelector(".response-toggle");
  if (room && roomToggle) {
    const setRoom = (active) => {
      room.dataset.state = active ? "active" : "standby";
      const v = room.querySelector(".response-visitor");
      const cu = room.querySelector(".response-curtains");
      const li = room.querySelector(".response-lights");
      const mo = room.querySelector(".response-mode");
      ...
      if (roomToggle.firstChild) roomToggle.firstChild.textContent = active ? "LEAVE ROOM " : "ENTER ROOM ";
    ...
    setRoom(false);
    roomToggle.addEventListener("click", () => setRoom(room.dataset.state !== "active"));
  }

  /* ---------- 6. §07 media gallery — 5 videos, exact filenames ---------- */
  const tabs = $$(".ob-media__tabs button");
  const frames = $$(".ob-media__frame");
  const stageEl = $(".ob-media__stage");
  if (tabs.length && frames.length) {
    /* missing-file detection: source error → elegant placeholder */
    frames.forEach((fr) => {
      const src = $("source", fr);
      const vid = $("video", fr);
    ...
    const activeVideo = () => $(".ob-media__frame.is-active video");
    tabs.forEach((tab) => tab.addEventListener("click", () => {
      ...
      /* user intent overrides the viewport gate — play immediately */
      const v = activeVideo();
      if (v) { v.load(); v.play?.().catch(() => {}); }
    }));
    /* IO gate: fetch on approach, play in view, pause out */
  ...
  if ("IntersectionObserver" in window) {
    ...
    $$(".reveal, .ob-reveal").forEach((el) => rObs.observe(el));
    const closing = $(".ob-closing");
    if (closing) {
      new IntersectionObserver(([e], obs) => {
        if (e.isIntersecting) { closing.classList.add("is-in"); obs.disconnect(); }
  ...
  if (roleBtns.length && roleOut) {
    roleBtns.forEach((btn) => btn.addEventListener("click", () => {
      ...
      roleOut.textContent = btn.dataset.copy || "";
    }));
  }
  ...
  if (purposeBtns.length) {
    ...
    purposeBtns.forEach((btn) => btn.addEventListener("click", () => {
      ...
      if (exp) exp.textContent = btn.dataset.experience || "";
      if (bus) bus.textContent = btn.dataset.business || "";
    }));
  }

  /* ---------- 10. architecture branches ---------- */
  ...
  if (arcBtns.length) {
    ...
    arcBtns.forEach((btn) => btn.addEventListener("click", () => {
      arcBtns.forEach((b) => b.classList.toggle("active", b === btn));
      if (outB) outB.textContent = btn.dataset.title || "";
      if (outP) outP.textContent = btn.dataset.what || "";
      if (outE) outE.textContent = btn.dataset.experience || "";
      if (outBus) outBus.textContent = btn.dataset.business || "";
    }));
  }

  /* ---------- 11. 30-sec dialog ---------- */
  const dialog = $(".summary-dialog");
  $$(".summary-open").forEach((btn) => btn.addEventListener("click", () => dialog?.showModal()));
