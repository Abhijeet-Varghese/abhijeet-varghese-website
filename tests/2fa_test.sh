...
mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -e "DELETE FROM login_attempts; DELETE FROM rate_limits; UPDATE users SET totp_enabled=0, totp_secret=NULL, totp_recovery=NULL WHERE email='admin@avos.test';" 2>/dev/null
...
A=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM audit_logs WHERE action='2fa_failed';" 2>/dev/null)
[ "$A" -ge 1 ] && ok "2FA_FAILED audited" || bad "audit fail" "$A"
CODE=$(php -r 'require getenv("APP") . "/backend/core/Totp.php"; echo Totp::code("'"$SECRET"'");')
...
A=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM audit_logs WHERE action='2fa_enabled';" 2>/dev/null)
[ "$A" -ge 1 ] && ok "2FA_ENABLED audited" || bad "audit enable" "$A"
# secret encrypted at rest
SEC=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT totp_secret FROM users WHERE email='admin@avos.test';" 2>/dev/null)
...
mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -e "UPDATE users SET totp_recovery=JSON_ARRAY('$(php -r 'echo password_hash("TESTREC1", PASSWORD_DEFAULT);')') WHERE email='admin@avos.test';" 2>/dev/null
...
A=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM audit_logs WHERE action='2fa_recovery_used';" 2>/dev/null)
...
A=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM audit_logs WHERE action='2fa_disabled';" 2>/dev/null)
[ "$A" -ge 1 ] && ok "2FA_DISABLED audited" || bad "audit disable" "$A"
ST=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT totp_enabled FROM users WHERE email='admin@avos.test';" 2>/dev/null)
