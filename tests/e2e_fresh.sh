...
T=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SHOW TABLES;" 2>/dev/null | wc -l)
[ "$T" -ge 62 ] && ok "table count >= 62 ($T)" || bad "table count $T < 62" ""
for t in leads opportunities meetings automations webhooks api_keys email_templates content_metrics; do
  mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SHOW TABLES LIKE '$t';" 2>/dev/null | grep -q "$t" && ok "table $t exists" || bad "table $t missing" ""
done
...
# the public website is the static frontend — there is no publish pipeline
R=$(curl -s -b $CJ $BASE/api/flags)
if echo "$R" | grep -q '"auto_publish"'; then bad "auto_publish flag must not exist" ""; else ok "no publish feature flags"; fi
...
echo "== 8. NO PUBLISH PIPELINE (static frontend) =="
CODE=$(curl -s -o /dev/null -w "%{http_code}" -b $CJ -X POST $BASE/api/publish -H "X-CSRF-Token: $CSRF" -d '{}')
check "POST /api/publish is gone (404)" "404" "$CODE"
CODE=$(curl -s -o /dev/null -w "%{http_code}" -b $CJ $BASE/api/deployments)
check "GET /api/deployments is gone (404)" "404" "$CODE"
CODE=$(curl -s -o /dev/null -w "%{http_code}" -b $CJ $BASE/api/redirects)
check "GET /api/redirects is gone (404)" "404" "$CODE"
R=$(curl -s $BASE/api/status)
check "status reports static site" '"site":"static"' "$R"
check "status reports public_site true" '"public_site":true' "$R"

echo "== 9. STATIC FRONTEND SERVED AS-IS =="
curl -s $BASE/ > /tmp/e2e_index.html
grep -q "\[E2E" /tmp/e2e_index.html && bad "CMS edit must NOT leak into the static site" "" || ok "CMS edit does not alter static homepage"
grep -q '/api/public/lead\|/api/analytics/track' /tmp/e2e_index.html $(ls $(dirname "$0")/../abhijeetvarghese/js/main.js) && ok "frontend wired to AV OS API" || bad "frontend API wiring missing" ""
for p in css/styles.css js/main.js robots.txt sitemap.xml search-index.json; do
  code=$(curl -s -o /dev/null -w "%{http_code}" $BASE/$p)
  [ "$code" = "200" ] && ok "$p 200" || bad "$p HTTP $code" ""
done
for p in index.html story.html portfolio.html contact.html insights.html journal.html consulting.html for-recruiters.html privacy-policy.html terms.html; do
  code=$(curl -s -o /dev/null -w "%{http_code}" $BASE/$p)
  [ "$code" = "200" ] && ok "$p 200" || bad "$p HTTP $code" ""
done
for d in experience/ case-studies/ case-studies/orange-business/ case-studies/indian-army/ case-studies/bharat-petroleum-corporation-limited/; do
  code=$(curl -s -o /dev/null -w "%{http_code}" $BASE/$d)
  [ "$code" = "200" ] && ok "/$d 200 (clean URL)" || bad "/$d HTTP $code" ""
done
R=$(curl -s -o /dev/null -w "%{http_code} %{redirect_url}" $BASE/case-studies.html)
check "legacy case-studies.html → 301 /case-studies/" "301 $BASE/case-studies/" "$R"
R=$(curl -s -o /dev/null -w "%{http_code} %{redirect_url}" $BASE/experience-design/bpcl-palakkad)
check "legacy bpcl slug → 301" "301 $BASE/case-studies/bharat-petroleum-corporation-limited/" "$R"
CODE=$(curl -s -o /dev/null -w "%{http_code}" $BASE/does-not-exist.html)
check "unknown page → 404" "404" "$CODE"
CODE=$(curl -s -o /dev/null -w "%{http_code}" $BASE/.htaccess)
check "dotfiles blocked (404)" "404" "$CODE"

echo "== 10. PUBLIC LEAD → CRM =="
R=$(curl -s -X POST $BASE/api/public/lead -H "Content-Type: application/json" -d '{"name":"E2E Visitor","email":"visitor@e2e.test","phone":"+919876543210","company":"E2E Corp","project_type":"experience centre","message":"Please contact me about an experience centre.","page":"/contact.html","utm_source":"e2e","utm_medium":"test","utm_campaign":"e2e-campaign","website":""}')
...
L=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT name,status,score,utm_campaign FROM leads WHERE id=$LID;" 2>/dev/null)
...
T=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM tasks;" 2>/dev/null)
check "automation task created" "1" "$T"
E=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM email_log;" 2>/dev/null)
...
U=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT status FROM leads WHERE id=$LID;" 2>/dev/null)
check "lead status persisted" "contacted" "$U"
AU=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM audit_logs WHERE action='lead_update';" 2>/dev/null)
...
M=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM meetings;" 2>/dev/null)
...
M=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM media WHERE id=$MID;" 2>/dev/null)
...
FS=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM form_submissions;" 2>/dev/null)
check "submission stored" "1" "$FS"
# backup
R=$(curl -s -b $CJ -X POST $BASE/api/backup -H "X-CSRF-Token: $CSRF" -d '{}')
check "backup created" '"ok":true' "$R"
B=$(ls "$(dirname "$0")/../avos-php/storage/backups/" 2>/dev/null | wc -l)
...
echo "== 15. CONTENT VERSIONS — RESTORE =="
curl -s -b $CJ $BASE/api/content > /tmp/e2e_doc.json
php -r '
$d = json_decode(file_get_contents("/tmp/e2e_doc.json"), true);
$doc = $d["data"];
...
DBT=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT data FROM content_store WHERE key_name='sections';" 2>/dev/null | grep -c 'E2E')
check "sections clean in DB" "0" "$DBT"
R=$(curl -s -b $CJ $BASE/api/versions/sections)
VER=$(echo "$R" | php -r '$d=json_decode(stream_get_contents(STDIN),true); $vs=$d["data"]??[]; usort($vs, fn($a,$b)=>$a["version"]<=>$b["version"]); foreach($vs as $v) if(stripos($v["note"]??"","saved from CMS")!==false){ echo $v["version"]; break; }')
[ -n "$VER" ] && ok "found E2E version v$VER" || bad "no CMS-saved version found" "$R"
R=$(curl -s -b $CJ -X POST $BASE/api/versions/sections/restore -H "Content-Type: application/json" -H "X-CSRF-Token: $CSRF" -d "{\"version\":$VER}")
check "restore ok" '"ok":true' "$R"
DBT2=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT data FROM content_store WHERE key_name='sections';" 2>/dev/null | grep -c 'E2E')
check "content restored in DB (E2E back)" "1" "$DBT2"
V2=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM versions WHERE entity='store' AND entity_id='sections' AND note LIKE 'restored%';" 2>/dev/null)
check "restore created a new version" "1" "$V2"

echo "== 16. STATIC SITE UNAFFECTED BY CMS CHANGES =="
curl -s -b $CJ -X PUT $BASE/api/content -H "Content-Type: application/json" -H "X-CSRF-Token: $CSRF" -d @/tmp/e2e_clean.json > /dev/null
curl -s -o /dev/null -w "%{http_code}" $BASE/ | grep -q 200 && ok "site live" || bad "site down" ""
C=$(curl -s $BASE/ | grep -c "\[E2E")
check "static homepage never carried CMS draft text" "0" "$C"
...
# SQL injection probe
R=$(curl -s -X POST $BASE/api/public/lead -H "Content-Type: application/json" -d '{"name":"x\" OR 1=1 --","email":"sqli@test.com","phone":"+919876543211","message":"probe"}')
...
for ep in "emailtemplates" "campaigns" "content-health" "ai/usage?days=30" "sites" "automations/runs" "webhooks" "apikeys" "flags" "knowledge" "errors" "emaillog"; do
...
echo "== 19. RBAC =="
# create a Viewer user and verify 403 on a privileged write
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" -b $CJ2 -X POST $BASE/api/backup -H "X-CSRF-Token: $CSRF2" -d '{}')
check "viewer backup → 403" "403" "$CODE"
...
echo
echo "== 20. CONTENT SAVE IS DATA-ONLY (no auto-publish) =="
curl -s -b $CJ $BASE/api/content > /tmp/e2e_doc.json
php -r '
$d = json_decode(file_get_contents("/tmp/e2e_doc.json"), true);
foreach ($d["data"]["sections"] as &$s) { if ($s["id"] === "hero") { $s["lede"] = "Data-only E2E lede"; } }
...
check "save ok" '"ok":true' "$R"
echo "$R" | grep -q auto_published && bad "save must not report auto_published" "$R" || ok "save response has no publish semantics"
C=$(curl -s $BASE/ | grep -c "Data-only E2E")
check "static site unchanged by CMS save" "0" "$C"
curl -s -b $CJ -X PUT $BASE/api/content -H "Content-Type: application/json" -H "X-CSRF-Token: $CSRF" -d @/tmp/e2e_clean.json > /dev/null
...
M=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM ai_agent_memory;" 2>/dev/null)
[ "$M" -ge 3 ] && ok "agent memory populated ($M observations)" || bad "memory" "$M"
J=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM ai_agent_jobs WHERE status='completed';" 2>/dev/null)
...
K=$(mysql -uavos -paV0s_d3v_9xKq2mN7 $DB -N -e "SELECT COUNT(*) FROM knowledge_items WHERE category IN ('journal-drafts','insight-drafts','case-study-drafts');" 2>/dev/null)
